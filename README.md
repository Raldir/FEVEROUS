...
Then install the package requirements specified in `src/requirements.txt`.
...
First you will need to make the src packages available to your python installation. This is done via the `PYTHONPATH=src` environment variable set before each command below. This can be skipped if the src folder has been installed in the previous step (See install with pip).
